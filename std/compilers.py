...
class Compiler_Codon(Compiler):
    ...
    def execute(self, tst):
        self.execute_monitor(tst, "--maxmem=2048:2048 ./program")
...
class Compiler_nodejs(Compiler):
    ...
    def execute(self, tst):
        ...
        cmd = "%s --basename=%s --maxprocs=100 --maxmem=2048:1024 -- /usr/bin/node program.js" % (monitor.path, tst)
...
class Compiler_Kotlin(Compiler):
    ...
    def compile(self):
        ...
        try:
            self.execute_compiler(
                "kotlinc " + self.flags1() + " program.kt -d program.jar -include-runtime > compilation1.txt"
            )
        except CompilationTooLong:
            util.write_file("compilation1.txt", "Compilation time exceeded")
            return False
...
class Compiler_Zig(Compiler):
    ...
    def compile(self):
        ...
        try:
            self.execute_compiler(
                "zig build-exe -femit-bin=./z" + self.flags1() + " program.zig 1> /dev/null 2> compilation1.txt"
            )
        except CompilationTooLong:
            util.write_file("compilation1.txt", "Compilation time exceeded")
            util.del_file("program")
